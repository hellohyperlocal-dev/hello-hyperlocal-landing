...
export function FoundingBusinesses() {
  return (
    <section id="founding-businesses" className="relative bg-hh-warm py-[100px] split:py-[130px]">
      <div className="site-container flex flex-col items-start gap-[50px]">
        <div className="flex w-full flex-col items-start gap-6 split:flex-row split:items-end split:justify-between">
          <div className="flex max-w-[720px] flex-col items-start gap-5">
            <SectionEyebrow label="Founding Businesses" tone="light" />
            <h2 className="m-0 type-h2 text-hh-onyx">Help shape Hello Linden before launch</h2>
        ...
        <ul className="m-0 grid w-full list-none grid-cols-1 gap-4 p-0 sm:grid-cols-2 xl:grid-cols-4 xl:gap-5">
          {HIGHLIGHTS.map(({ icon: Icon, title, body }) => (
            <li key={title} className="flex flex-col items-start gap-4 rounded-card bg-white p-6">
              <span className="flex h-11 w-11 items-center justify-center rounded-button bg-hh-forest text-hh-lime">
                <Icon aria-hidden className="h-5 w-5" strokeWidth={2.2} />
              </span>
              <span className="type-h3 text-hh-onyx">{title}</span>
              <span className="type-body text-hh-muted">{body}</span>
            </li>
          ))}
        </ul>
      </div>
